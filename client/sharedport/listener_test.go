...
func TestPassSockHeaderRejects(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			buf:  buildPassSockFrame(99 /* != SHARED_PORT_PASS_SOCK (76) */),
...
// shortSocketPath returns a UDS path guaranteed to fit within the
// platform's sockaddr_un.sun_path limit (104 bytes on macOS, 108 on
// Linux). t.TempDir() roots paths under long per-test directories
// (e.g. /var/folders/.../TestListenerCloseWaitsForHandlers123/001) that
// can exceed the macOS limit and make net.ListenUnix fail with
// "bind: invalid argument". We allocate a short os.MkdirTemp dir
// instead and clean it up via t.Cleanup.
func shortSocketPath(t *testing.T) string {
	...
	return filepath.Join(dir, "ep.sock")
}
