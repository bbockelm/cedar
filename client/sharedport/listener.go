...
// Listener is the endpoint side of HTCondor's shared-port forwarding
// mechanism: the side that *receives* connections forwarded by a
// condor_shared_port daemon.
//
// A condor_shared_port daemon lets many daemons share one network port.
// When it decides a connection belongs to a particular endpoint, it
// hands the connected client fd off over a Unix-domain socket using a
// SHARED_PORT_PASS_SOCK handshake followed by SCM_RIGHTS fd passing.
// This is the general shared-port endpoint mechanism — it is not
// specific to any particular application protocol (HTTP, CEDAR, or
// otherwise); whatever bytes arrive on the forwarded fd are presented
// untouched.
//
// Listener accepts those forwarded fds and exposes each one as a
// net.Conn. It implements net.Listener so callers can plug it directly
// into anything expecting a net.Listener (for example http.Server.Serve
// / ServeTLS), treating forwarded connections like any other accepted
// connection.
