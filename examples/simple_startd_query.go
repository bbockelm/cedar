...
func main() {
	...
	if err != nil {
		slog.Error(fmt.Sprintf("Failed to serialize query: %v", err))
	}

	fmt.Printf("📏 Query message size: %d bytes\n", len(queryData))

	if err := cedarStream.SendMessage(ctx, queryData); err != nil {
		slog.Error(fmt.Sprintf("Failed to send query: %v", err))
	}
	...
	if err != nil {
		slog.Error(fmt.Sprintf("Failed to receive response: %v", err))
	}
	...
	if err != nil {
		slog.Error(fmt.Sprintf("Failed to read numExprs: %v", err))
	}
	...
	for numExprs > 0 {
		...
		for i := 0; i < int(numExprs); i++ {
			...
			if err != nil {
				slog.Error(fmt.Sprintf("Failed to read expression %d: %v", i, err))
			}
			fmt.Printf("   Expr %d: %s\n", i+1, exprStr)
		}
